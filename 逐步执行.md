...
可以看到，在**Threads**窗口，蓝色的小箭头标识当前选中是哪一个线程，上图选中的是主线程。所有逐步执行的操作，包括**resume/pause/step over/step into**等等均属于当前选中的线程。换句话说，当你在上图所示的状态，点击了**Resume script execution**摁钮，主线程会恢复执行，而工作线程依旧处于中断状态。在选中主线程时，**Call Stack(调用栈)**和**Scope(作用域)**窗口也只显示主线程的信息。如果想查看工作线程相关的信息，或者想在工作线程中逐步执行，只需要在**Threads**窗口单击工作线程的标签，将环境切换到工作线程即可。
