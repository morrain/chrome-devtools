...
--- | ---
代码行断点 | 知道设置断点的精确位置时
条件断点 | 知道设置断点的精确位置并且希望当满足特定条件才中断时
DOM断点 | 当特定DOM节点发生变化、被移除或者子节点被移除时，可以中断到引起变化的代码
XHR断点 | 当XHR URL包含指定字符串时，中断到发送请求的代码位置
事件断点 | 当特定事件触发时中断
异常断点 | 当希望在抛出异常时中断到抛出异常的地方时
...
源文件左侧是**行号槽**，这里显示了源代码的行号，在行号槽上点击便在该行上设置了代码行断点。如下图所示，在29行成功设置了断点。
...
停用断点表示调试器暂时忽略该断点，日后需要时可以重新启动该断点，对于确定不需再次启用的断点，你可以删除它们。
...
谷歌开发者工具一共提供了两种类型的XHR断点：一种是当XHR生命周期到某种状态时触发的XHR状态断点，还有一种是XHR的请求URL中包括匹配的字符串时，当XHR调用send()时，会中断下来。
